...
class Dataset(metaclass=ABCMeta):
    ...
    def __init__(self, dataset, columns_mapping, column_to_add_to_group):
        ...
        self.column_to_add_to_group = column_to_add_to_group
    ...
    def _map_column_names(self, row):
        ...
        for key, n_key in zip([getattr(k, self.dataset_source) for k in self.columns_mapping.values()],
                              self.columns_mapping.keys()):
            row[n_key] = row.pop(key)

    def _clean_row(self, row):
        """Cleans a row getting rid of the unwanted columns."""
        unwanted_columns = set(row.keys()) - set(self.columns_mapping.keys())
